...
import { Home, BookOpen, Video, Calendar, User, LogOut, Menu, X, Bell, Settings, Megaphone, UserPlus, Sun, Moon, CheckCheck, Gift, UsersRound, Sparkles, GraduationCap, Search, ScanSearch } from 'lucide-react';
...
export function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  const getTenantLogo = () => tenantSettings?.logo_url || currentTenant?.logo_url || undefined;
  ...
  const navItems = [
    ...
    { icon: GraduationCap, label: language === 'he' ? 'מסלול הלמידה' : 'Learning Path', path: '/learning-path' },
  ...
  const instructorPlusNavItems = [
    ...
    { icon: ScanSearch, label: language === 'he' ? 'תוצאות אישיות' : 'Personality Results', path: '/admin/personality-results' },
  ];

  // Items visible only to admins/super_admins.
  const adminOnlyNavItems = [
    { icon: Sparkles, label: t('nav.skillsLibrary'), path: '/skills' },
  ...
  return (
    <div className="h-screen bg-background flex w-full overflow-hidden">
      ...
      {/* Sidebar */}
      <aside className={cn(
        "fixed inset-y-0 right-0 z-50 w-64 bg-sidebar text-sidebar-foreground transition-transform duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] shadow-xl",
          ...
          <div className="h-16 flex items-center justify-between px-4 border-b border-sidebar-border/50">
            <div className="flex items-center gap-3 w-full">
              <Avatar className="w-10 h-10 rounded-lg flex-shrink-0">
                {getTenantLogo() && <AvatarImage src={getTenantLogo()} className="object-contain" />}
                <AvatarFallback className="rounded-lg bg-sidebar-primary text-sidebar-primary-foreground text-sm font-bold">
                  {currentTenant?.name ? getTenantInitials(currentTenant.name) : 'T'}
                </AvatarFallback>
              </Avatar>
      ...
      {/* Main content */}
      <div className="flex-1 flex flex-col min-w-0 lg:mr-64 overflow-hidden">
        ...
        <header className="h-16 bg-card/80 backdrop-blur-xl border-b border-border/60 flex items-center justify-between px-4 lg:px-6 z-30 flex-shrink-0">
          <div className="flex items-center gap-4">
            <Button
              ...
              aria-label={language === 'he' ? 'פתיחת תפריט' : 'Open menu'}
            >
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              aria-label={language === 'he' ? 'פתיחת חיפוש מהיר' : 'Open quick search'}
            >
              <Search className="w-3.5 h-3.5" />
              <span>{language === 'he' ? 'חיפוש' : 'Search'}</span>
            ...
            <Popover open={notificationsOpen} onOpenChange={setNotificationsOpen}>
              <PopoverTrigger asChild>
                <Button
                  ...
                  aria-label={
                    totalUnread > 0
                      ? `${language === 'he' ? 'התראות' : 'Notifications'} (${totalUnread})`
              ...
              <PopoverContent align="end" className="w-80 p-0">
                <div className="p-3 border-b border-border">
                  <div className="flex items-center justify-between gap-2">
                    <h4 className="font-semibold text-sm">{language === 'he' ? 'התראות' : 'Notifications'}</h4>
                    <div className="flex items-center gap-2">
                ...
                <ScrollArea className="h-[340px]">
                  ...
                  {announcements.length === 0 && smartNotifications.length === 0 ? (
                    <div className="p-6 text-center text-muted-foreground">
                      ...
                      <p className="text-sm">{language === 'he' ? 'אין התראות חדשות' : 'No new notifications'}</p>
                    </div>
                            ...
                            <div className="flex items-start gap-3">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-muted-foreground mt-1">
                                  {format(new Date(announcement.created_at), 'd בMMM', { locale: language === 'he' ? he : enUS })}
                                </p>
                              </div>
                            </div>
                          </Link>
